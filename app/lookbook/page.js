      ...
      <div style={{ padding:'60px 80px', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:3 }}>
        {PRODUCTS.map(p => (
          <Link key={p.id} href={`/shop/${p.id}`} style={{ background:'#0f0b0e', display:'block' }}>
            <div style={{ aspectRatio:'1/1', background:'linear-gradient(160deg,rgba(66,4,32,0.2),#0d0808)', display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
              {p.image ? <img src={p.image} alt={p.name} style={{ width:'100%', height:'100%', objectFit:'contain', objectPosition:'center', display:'block' }} /> : <span style={{ fontSize:56, opacity:0.25 }}>{p.emoji}</span>}
