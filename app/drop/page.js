    ...
    <main style={{ background: '#080808', color: '#fdf8fc', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', padding: '110px 24px 84px', textAlign: 'center', overflow: 'hidden', borderBottom: '1px solid rgba(255,173,237,0.1)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 820, margin: '0 auto' }}>
          ...
          <h1 style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 'clamp(60px,11vw,140px)', lineHeight: 0.9, letterSpacing: '0.02em', marginBottom: 14 }}>The Faded<br /><span style={{ color: 'transparent', WebkitTextStroke: '1px #FFADED' }}>Capsule</span></h1>
          <p style={{ fontFamily: 'Cormorant Garamond,serif', fontSize: 20, fontStyle: 'italic', color: '#9a7a8e', marginBottom: 34, letterSpacing: '0.05em' }}>Three pieces. One statement. Just be Blunt.</p>
          ...
          {capsule.map(p => (
            <Link key={p.id} href={`/shop/${p.id}`} style={{ position: 'relative', background: '#0f0b0e', overflow: 'hidden', display: 'block', textDecoration: 'none', color: '#fdf8fc' }}>
              <div style={{ aspectRatio: '3/4', background: 'linear-gradient(160deg, rgba(66,4,32,0.25) 0%, #0d0808 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow:'hidden' }}>
                {p.image ? <img src={p.image} alt={p.name} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <span style={{ fontSize: 60, opacity: 0.25 }}>{p.emoji}</span>}
              ...
              <div style={{ padding: '20px 24px 28px', borderTop: '1px solid rgba(255,173,237,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>{p.colors.map(c => <div key={c.hex} style={{ width: 14, height: 14, borderRadius: '50%', background: c.hex, border: '1px solid rgba(255,255,255,0.15)' }} />)}</div>
                </div>
